...
This notice covers data processed by the Blocs web app and API routes.
...
- On-chain data is public by design and may remain permanently accessible.
...
- Abuse-prevention identifiers:
  - trusted client IP (when trusted proxy headers are present), or
...
- Browser context (`href`, `userAgent`) and timestamp.
...
- To debug production issues when client-error reporting is enabled.
...
- Client error forwarding (optional): `ERROR_REPORT_WEBHOOK_URL` destination.
...
- Retention for uploaded assets and forwarded webhook logs depends on your infrastructure configuration.
...
For privacy or data-handling questions, open a GitHub issue.
